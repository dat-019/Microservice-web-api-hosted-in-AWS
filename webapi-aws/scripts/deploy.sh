...
eval $(aws ecr get-login-password --region $AWS_DEFAULT_REGION | sed 's|https://||') #needs AWS_ACCESS_KEY_ID and AWS_SECRET_ACCESS_KEY envvars

#eval $(aws ecr get-login-password --region us-east-2 | docker login --username AWS --password-stdin $AWS_ECS_REPO_DOMAIN/$IMAGE_NAME)

# image1
if [$(aws ecr describe-repositories | jq --arg x $IMAGE_NAME1 '[.repositories[] | .repositoryName == $x] | any') == "true"]; then
    echo "Found ECS Repository $IMAGE_NAME1"
else
    echo "ECS Repository doesn't exist, Creating $IMAGE_NAME1 ..."
    aws ecr create-repository --repository-name $IMAGE_NAME1
fi
docker push $AWS_ECS_REPO_DOMAIN/$IMAGE_NAME1:$IMAGE_VERSION

# image2
if [$(aws ecr describe-repositories | jq --arg x $IMAGE_NAME2 '[.repositories[] | .repositoryName == $x] | any') == "true"]; then
    echo "Found ECS Repository $IMAGE_NAME2"
else
    echo "ECS Repository doesn't exist, Creating $IMAGE_NAME2 ..."
    aws ecr create-repository --repository-name $IMAGE_NAME2
fi
